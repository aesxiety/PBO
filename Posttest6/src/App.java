...
public class App {
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    String KodeOthersProduct = "OT" + othersProduct.size() + 1;
                    System.out.print("Enter Product Name: ");
    ...
    private static void manageOrdersMenu() throws IOException{
        ...
        while (true) {
            ...
            System.out.println("+----------------------------------------------+");
            System.out.println("| [1] Show Order    | [5] Calculate Discount   |");
            System.out.println("| [2] Add New Order | [6] Set Pengiriman       |");
            System.out.println("| [3] Update Order  | [7] Update Pengiriman    |");
            System.out.println("| [4] Delete Order  | [8] Exit                 |");
            System.out.println("+----------------------------------------------+");
            System.out.print("| Choose Menu : ");
            int choice = Integer.parseInt(br.readLine());
            switch (choice) {
    ...
    private static void setPengiriman() throws IOException{
        showOrder(orders);
        System.out.println("Masukkan Kode Pesanan:");
        String kodeOrderInput = br.readLine();

        Order order = null;
        for (Order pesanan : orders) {
            if (pesanan.getOrderId().equals(kodeOrderInput)) {
                order = pesanan;
                break;
            }
        }
        ...
        if (order != null) {
            ...
            // Meminta input dari pengguna untuk informasi pengiriman tambahan
            System.out.println("Masukkan Alamat Asal:");
            String alamatAsal = br.readLine();

            System.out.println("Masukkan Alamat Tujuan:");
            String alamatTujuan = br.readLine();

            System.out.println("Masukkan Ongkir:");
            double ongkir = Double.parseDouble(br.readLine());

            // Membuat objek PengirimanBarang dengan informasi yang dimasukkan pengguna
            pengirimanBarang kirim = new pengirimanBarang(order, alamatAsal, alamatTujuan, ongkir);

            // Memanggil metode buatPengiriman() dari objek pengirimanBarang
            kirim.buatPengiriman();

        } else {
            System.out.println("Pesanan dengan kode " + kodeOrderInput + " tidak ditemukan.");
        }

    } 
    private static void updatePengiriman() throws IOException{
        showOrder(orders);
        System.out.println("Masukkan Kode Pesanan:");
        String kodeOrderInput = br.readLine();
        ...
        // Memeriksa apakah pesanan ditemukan
        if (order != null) {
            pengirimanBarang pengirmn = new pengirimanBarang(order, kodeOrderInput, kodeOrderInput, 0);
            pengirmn.ubahStatusPengiriman();
    ...
    private static void addOrder(ArrayList<Order> orders, BufferedReader br, ArrayList<Product> products) throws IOException {
        System.out.println("List Product By Category");
        ShowKaosProduct();
        ShowKemejaProduct();
        ShowOthersProduct();
        System.out.println("Enter Product Code : ");
        String searckode = br.readLine();
        String itemP = "";
        boolean productFound = false;
        
        for (int i = 0; i < kemejaProduct.size(); i++) {
            Kemeja kmjpr = kemejaProduct.get(i);
            if (kmjpr.getKodeProduk().equals(searckode)) {
                itemP = kmjpr.getNamaProduk();
                productFound = true;
                break; // Keluar dari loop setelah menemukan produk yang sesuai
            }
        }
        for (int i = 0; i < kaosProduct.size(); i++) {
            Kaos kaospr = kaosProduct.get(i);
            if (kaospr.getKodeProduk().equals(searckode)) {
                itemP = kaospr.getNamaProduk();
                productFound = true;
                break; // Keluar dari loop setelah menemukan produk yang sesuai
            }
        }
        for (int i = 0; i < othersProduct.size(); i++) {
            ProdukLainnya ot = othersProduct.get(i);
            if (ot.getKodeProduk().equals(searckode)) {
                itemP = ot.getNamaProduk();
                productFound = true;
                break; // Keluar dari loop setelah menemukan produk yang sesuai
            }
        }
        ...
        if (productFound==true) {
            String orderId = "ORD10" + orders.size();
            String nameProdct = itemP;
            System.out.print("Enter Customer Name: ");
            String customerName = br.readLine();
            System.out.print("Enter Quantity: ");
            int quantity = Integer.parseInt(br.readLine());
            Order newOrder = new Order(orderId, customerName, nameProdct, quantity);
            orders.add(newOrder);
